    ...
    if info.get("telegram_direct"):
        ...
        if finfo.get("ok"):
            ...
            if range_header and file_size > 0:
                ...
                # 🛠️ TELEGRAM MTPROTO [400 OFFSET_INVALID] FIX: Aligned 1MB Chunk Math (1MB = 1048576 bytes)
                chunk_size = 1048576
                aligned_offset = (byte1 // chunk_size) * chunk_size
                skip_bytes = byte1 - aligned_offset
                
                def generate_range_stream():
                    bytes_sent = 0
                    skip = skip_bytes
                    try:
                        for chunk in pyro_client.stream_media(info["telegram_file_id"], offset=aligned_offset):
                            if skip > 0:
                                if len(chunk) <= skip:
                                    skip -= len(chunk)
                                    continue
