            ...
            if res.status_code == 200:
                ...
                for update in data.get("result", []):
                    ...
                    if text.startswith("/start"):
                        ...
                        srv_url = global_server_url or f"http://localhost:{PORT}"
                    ...
                    # 2. Handle Media Uploads (No command required!)
                    ...
                    if media:
                        ...
                        send_telegram_direct(chat_id, f"⏳ Downloading <code>{orig_name}</code> to MeetLink cloud disk...")
                        
                        finfo = requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/getFile", params={"file_id": file_id_tg}, timeout=15).json()
                        if finfo.get("ok"):
                            file_path_tg = finfo["result"]["file_path"]
                            dl_url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{file_path_tg}"
                            
                            uid = generate_unique_id(8)
                            local_path = os.path.join(UPLOAD_DIR, uid)
                            
                            r_file = requests.get(dl_url, stream=True, timeout=60)
                            ...
                            file_store[uid] = {
                                "fileName": orig_name,
                                "fileSize": fmt_size(file_size or os.path.getsize(local_path)),
                                "fileSizeBytes": file_size or os.path.getsize(local_path),
...
def upload_recording():
    ...
    try:
        ...
        # Save original WebM to temporary disk
        ...
        # Submit to worker thread pool jisse response turant (5ms) frontend ko wapas mil jaye
        executor.submit(_bg_process_recording, webm_path, room_id, seg_num, is_last, timestamp, webm_size, part_label)
...
def upload_shared_file():
    ...
    try:
        ...
        executor.submit(_bg_send_telegram_file, file_path, original_name, caption)
...
<form method="GET"><input type="password" name="pwd" placeholder="Enter 4-digit PIN / Password" required autofocus><button type="submit">Unlock & Download</button></form></div></body></html>''', 401
    ...
    return send_file(fp, download_name=info.get("fileName", "file"), as_attachment=True)
