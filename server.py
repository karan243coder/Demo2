...
def convert_webm_to_mp4(input_path, output_path):
    """Convert WebM recording to MP4 (H264 + AAC) for Telegram playback"""
    try:
        cmd = [
        ...
        if result.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            print(f"✅ MP4 converted: {fmt_size(os.path.getsize(output_path))}")
            return True
        else:
            print(f"❌ MP4 conversion failed: {result.stderr[:200]}")
            return False
    except subprocess.TimeoutExpired:
        print("❌ MP4 conversion timed out")
        return False
    except Exception as e:
...
def extract_mp3_from_video(input_path, output_path):
    ...
    try:
        ...
        if result.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            print(f"✅ MP3 extracted: {fmt_size(os.path.getsize(output_path))}")
            return True
        else:
            print(f"❌ MP3 extraction failed: {result.stderr[:200]}")
            return False
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print(f"🌐 Port: {PORT}")
    print(f"🎬 FFmpeg: {'✅ Available' if os.path.exists('/usr/bin/ffmpeg') else '❌ Not found'}")
    print("=" * 50)
